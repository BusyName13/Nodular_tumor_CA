...
if __name__ == "__main__":
    ...
    simulation_generator.generate_params_and_make_sims(param_values, 'parameters', 'simulation_results', 'parameters_')

    print('Strting analysis:')
    ...
    for i, params in enumerate(param_values):
        # запуск модели
        fname = f"simulation_results/result_parameters_{i}.txt"
        if not os.path.isfile(fname):
            print(f'Warning! file {fname} was NOT found')
            continue
        
        # if os.path.isfile(f"simulation_results/result_parameters_{i}.txt"):
        #     image = np.loadtxt(f"simulation_results/result_parameters_{18}.txt", dtype=int)
        # else:
        # image = simulation_generator.make_sim(f"parameters/parameters_{i}.txt", f"simulation_results/result_parameters_{i}.txt")
        image = simulation_generator.load_data(fname)
        ...
        if i % 10 == 0:
            print(i, fd, lac, sep='\t', end='\t')
            end_time = time.perf_counter()
            print(f"t = {end_time-start_time:.2f}")
            start_time = end_time
