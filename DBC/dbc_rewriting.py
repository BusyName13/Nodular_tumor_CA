...
def DBC(img_name):
    ...
    img_size = min(img.shape) - 1
    img = img[0:img_size - 1, 0:img_size - 1] #обрезали до квадрата
    if min(img.shape) != max(img.shape): print("!!! Изображение было обрезано до квадратного.")

    G = 256 #количество оттенков

    dots = []
    for s in get_divisors_reverse(img_size): #для каждого размера s ячейки
        curr_img = img #дополнение нулями, надо найти лучшую модификациюю этого момента
        ...
        for i in range(s, curr_size + 1, s): #для каждой ячейки размера s
            for j in range(s, curr_size + 1, s):
                ...
                g_max, g_min = int(box.max()), int(box.min())
                k = g_min // h + 1
                l = g_max // h + 1
